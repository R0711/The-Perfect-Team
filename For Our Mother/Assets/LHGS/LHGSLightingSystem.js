...
selectQualitytModel();
	...
	if(baseParameters.DirectLight.CreateDirectLight == true)
	{
    	...
    	directLight.light.type = LightType.Directional;
    	directLight.light.shadows = LightShadows.Soft;
    	directLight.light.color = Color32(167, 124, 44, 255);
    	directLight.light.intensity = 0.5;
		}
...
function LHGS()
{
	...
	LightmapEditorSettings.resolution = rendering.LightMapResolution;
	...
	if(lightDistributionMethod.solvingIrregular == true){
		...
		for(var rayIV = skySolver; rayIV < 72/lightingSolverCustomFactor*1.5; rayIV++)
		{
			if(( rayIV < 48/lightingSolverCustomFactor)&&(rayIV > skySolver/lightingSolverCustomFactor)){
    			...
    			LHGSLight.light.type = LightType.Directional;
    			LHGSLight.light.shadows = LightShadows.Soft;
    			LHGSLight.hideFlags = HideFlags.HideInInspector;
    			LHGSLight.hideFlags = HideFlags.HideInHierarchy;
    			LHGSLight.light.color = baseParameters.SkySettings.LHGS_SkyColor;
    ...
    for(var LHGSLightsIntensityChange : GameObject in GameObject.FindObjectsOfType(GameObject))
	{
    	...
    	{
    	LHGSLightsIntensityChange.light.intensity = (((baseParameters.SkySettings.LHGS_Intensity/1.5)))/lightingModelQuality.sourceOfRays;
